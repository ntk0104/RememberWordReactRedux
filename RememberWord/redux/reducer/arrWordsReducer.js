...
const arrWordsReducer = (state = defaultArrWord, action) => {
    if(action.type === 'CHANGE_MEMORIZED_STATUS'){
        return state.map(e => {
    ...
    if(action.type === 'CHANGE_DISPLAY_STATUS'){
        return state.map(e => {
            if(e.id !== action.id) return e
            return {
                ...e, isShow: !e.isShow
    ...
    if(action.type === 'ADD_NEW_WORD'){
        return [{
            id: state.length + 1, en: action.en, vn: action.vn, memorized: false, isShow: false
        }].concat(state)
    }
    return state
}
